<template>
  <view
    class="cmd-bottom-nav"
    :class="[textAuto ? 'cmd-bottom-nav-text-auto':'',fixed ? 'cmd-bottom-nav-fixed':'']"
    :style="setColorStyle"
  >
    <view
      :class="['cmd-bottom-nav-box',select == index ? 'cmd-bottom-nav-active':'']"
      :style="select == index ? setActiveFontColorStyle : ''"
      @tap="$_click(index)"
    ...
    >
      <view class="cmd-bottom-nav-box-icon">
        <cmd-icon
          v-if="nav.icon && !nav.src"
          :type="nav.icon"
          size="24"
          :color="select == index ?activeFontColor :  fontColor"
        ></cmd-icon>
      </view>
      <image
        v-if="nav.src && !nav.icon"
        class="cmd-bottom-nav-box-img"
        :src="select == index ? nav.srcSelect:nav.src"
        mode="aspectFit"
      ></image>
